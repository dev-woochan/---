...
//GET에서 받은 종목명만 조회하기 
    if(isset($_GET['searchStock'])){ //검색값이 있을때 
      ...
      $countSql = "SELECT COUNT(*) AS row_count FROM koPost WHERE stockName = '{$searchStock}'";
      ...
      WHERE koPost.stockName = '{$searchStock}'
      ...
      if($result){
          ...
          mysqli_error($mysqli);
            ...
            $countSql = "SELECT COUNT(*) AS row_count FROM koPost";
            $result = $mysqli->query($countSql);
    ...
    <main>
        ...
        <div class="bottom_paging_wrap" style="margin-top:20px;">
            <div class="bottom_paging_box_iconpaging">
                ...
                echo "<a href='board_koreaStock.php?currentPage=1'>처음</a>";
                echo "<a href='board_koreaStock.php?currentPage=${prevPage}'>이전</a>";
                 
                for($index =1; $index<=$totalPage; $index++){
                    echo "<a href='board_koreaStock.php?currentPage=${index}'>${index}</a>";
                }
                if($totalPage ==1){
                    $nextpage = 1;
                }else{
                ...
                echo "<a href='board_koreaStock.php?currentPage=${nextpage}'>다음</a>";
                echo "<a href='board_koreaStock.php?currentPage=${totalPage}'>끝</a>"
                ?>                   
            </div>
