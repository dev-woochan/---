...
if (isset($data['parent_comment_id'])) { //대댓글의 경우!
    ...
    $parentSql = "SELECT id, orderNumber FROM comment WHERE id = ?";
    ...
    $stmt = $mysqli->prepare("INSERT INTO comment (create_time, content, parentPostId, orderNumber, depth, user_id, deleted, parentCommentId, likes, replyOrder ) 
VALUE (?, ?, ?, ?, ?, ?, ?, ?, ?, ?)");
...
";
    ...
    $orderNumberSql = "SELECT MAX(orderNumber) AS max_value FROM comment WHERE parentPostId = $parentPostId";
    ...
    $stmt = $mysqli->prepare("INSERT INTO comment (create_time, content, parentPostId, orderNumber, depth, user_id, deleted, likes) 
VALUE (?, ?, ?, ?, ?, ?, ?, ?)");
