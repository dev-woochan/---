...
$checkComment = "SELECT parentCommentId, orderNumber FROM comment WHERE id = $comment_id";
//댓글, 대댓글여부 확인을 위함 
...
if ($check == NULL) { //부모 댓글의 경우 
    $deleteSql = "UPDATE comment SET deleted = 1 WHERE id = ?";
    //deleted = 1로 바꿈 
    ...
    $selectSql = "SELECT COUNT(deleted) AS deleted FROM comment WHERE orderNumber = {$orderNumber} && deleted = 0";
    ...
    $selectSql = "SELECT deleted, depth FROM comment WHERE orderNumber = {$orderNumber}";
    $selectResult = mysqli_query($mysqli, $selectSql);
    $cnt = 0;
    ...
    if ($cnt > 2) { //두개 이상일시 => 그냥 삭제만 하면됨 
        ...
        if ($parentdeleted == 1) {
            //삭제되어있는경우 
            $delete = "DELETE FROM comment WHERE orderNumber = ?";
            $stmtd = $mysqli->prepare($delete);
