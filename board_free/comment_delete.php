...
$checkComment = "SELECT parentCommentId, orderNumber FROM freeComment WHERE id = $comment_id";
//댓글, 대댓글여부 확인을 위함 
...
if ($check == NULL) { //부모 댓글의 경우 
    $deleteSql = "UPDATE freeComment SET deleted = 1 WHERE id = ?";
    //deleted = 1로 바꿈 
    ...
    $selectSql = "SELECT COUNT(deleted) AS deleted FROM freeComment WHERE orderNumber = {$orderNumber} && deleted = 0";
    ...
    if ($deletedCount > 1) { //대댓글이 있을때 
        ...
        $delete = "DELETE FROM freeComment WHERE id = ?";
    ...
    $selectSql = "SELECT deleted, depth FROM freeComment WHERE orderNumber = {$orderNumber}";
    $selectResult = mysqli_query($mysqli, $selectSql);
    $cnt = 0;
    ...
    if ($cnt > 2) { //두개 이상일시 => 그냥 삭제만 하면됨 
        $delete = "DELETE FROM freeComment WHERE id = {$comment_id}";
        ...
        if ($parentdeleted == 1) {
            //삭제되어있는경우 
            $delete = "DELETE FROM freeComment WHERE orderNumber = ?";
            $stmtd = $mysqli->prepare($delete);
